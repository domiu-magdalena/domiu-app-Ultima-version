-- ============================================
-- DOMIU MAGDALENA - SETUP COMPLETO V2
-- Primero agrega columnas faltantes, luego datos
-- ============================================
...
ALTER TABLE negocios ADD COLUMN IF NOT EXISTS usuario_id UUID;
ALTER TABLE negocios ADD COLUMN IF NOT EXISTS local_id UUID;
...
ALTER TABLE negocios ADD COLUMN IF NOT EXISTS domicilio_cost NUMERIC(10,2) DEFAULT 0;
ALTER TABLE negocios ADD COLUMN IF NOT EXISTS horario TEXT DEFAULT '';
...
-- Productos
ALTER TABLE productos ADD COLUMN IF NOT EXISTS categoria_producto TEXT DEFAULT 'General';
...
CREATE TABLE IF NOT EXISTS pedidos_cliente (
  ...
  recogido_en TIMESTAMPTZ, entregado_en TIMESTAMPTZ,
...
CREATE TABLE IF NOT EXISTS calificaciones (
  ...
  pedido_interno_id UUID, tipo TEXT NOT NULL CHECK (tipo IN ('repartidor', 'negocio', 'producto')),
  repartidor_id UUID, negocio_id UUID REFERENCES negocios(id) ON DELETE SET NULL,
...
SELECT * FROM (VALUES
  ('Pizzeria Napoli', 'Restaurantes', 'Las mejores pizzas artesanales de Magdalena.', 'Calle Real #123, Magdalena', '3001112233', 4.8, '25-35 min', 3500, true, true, true),
  ('Super Tienda Don Pepe', 'Tiendas', 'Todo para tu hogar.', 'Av. Central #456, Magdalena', '3001113344', 4.3, '20-30 min', 3000, true, true, true),
  ('Licores El Buen Gusto', 'Licoreras', 'Cervezas, vinos y licores.', 'Carrera 7 #789, Magdalena', '3001114455', 4.6, '30-40 min', 4000, true, false, true),
  ('Drogueria Salud Total', 'Droguerias', 'Medicamentos y cuidado personal.', 'Calle 12 #321, Magdalena', '3001115566', 4.7, '15-25 min', 2500, true, false, true),
  ('Sushi Master', 'Restaurantes', 'Sushi fresco y comida japonesa.', 'Av. del Rio #654, Magdalena', '3001116677', 4.9, '35-50 min', 4500, true, true, true)
...
CROSS JOIN (VALUES
  ('Pizza Margherita','Mozzarella fresca',22000,'Pizzas'),
  ('Pizza Pepperoni','Pepperoni',25000,'Pizzas'),
  ('Pizza Hawaiana','Piña',24000,'Pizzas'),
  ('Pizza Vegetariana','Veggie',23000,'Pizzas'),
  ('Coca-Cola 1.5L','Gaseosa',5000,'Bebidas')
...
CROSS JOIN (VALUES
  ('Arroz Diana 1kg','Arroz',3500,'Abarrotes'),
  ('Aceite Vegetal 1L','Aceite',8500,'Abarrotes'),
  ('Leche Entera 1L','Leche',4200,'Lacteos'),
  ('Pan Bimbo Grande','Pan',6800,'Panaderia'),
  ('Jabon para Ropa','Detergente 500g',4500,'Limpieza')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);
...
CROSS JOIN (VALUES
  ('Cerveza Aguila x6','Pack 6',18000,'Cervezas'),
  ('Vino Tinto Casillero','750ml',45000,'Vinos'),
  ('Ron Medellin Anejo','750ml',55000,'Licores'),
  ('Coctel Margarita','500ml',22000,'Cocteles')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);
...
CROSS JOIN (VALUES
  ('Acetaminofen 500mg x10','Analgesico',2500,'Medicamentos'),
  ('Ibuprofeno 400mg x10','Antiinflamatorio',3500,'Medicamentos'),
  ('Curitas x20','Apósito',2000,'Cuidado Personal'),
  ('Alcohol Antiseptico 500ml','Alcohol 70%',4500,'Cuidado Personal')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);
...
CROSS JOIN (VALUES
  ('Roll Salmón Philadelphia','Salmon, queso crema',28000,'Rolls'),
  ('California Roll','Cangrejo, aguacate',24000,'Rolls'),
  ('Nigiri Mix x8','8 piezas',32000,'Nigiri')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);
