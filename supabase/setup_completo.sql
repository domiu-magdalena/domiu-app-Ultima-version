...
-- Crear pedidos_cliente (solo si no existe)
...
-- Agregar metodo_pago y calificaciones (si no existen)
...
-- Agregar columnas faltantes a negocios
...
DROP POLICY IF EXISTS "negocios_delete_public" ON negocios;
CREATE POLICY "negocios_delete_public" ON negocios FOR DELETE USING (true);
...
-- Pedidos: todos pueden insertar/leer/actualizar
...
-- Detalle pedido: todos pueden insertar/leer
...
CREATE POLICY "locales_select_public" ON locales FOR SELECT USING (true);
DROP POLICY IF EXISTS "locales_insert_public" ON locales;
CREATE POLICY "locales_insert_public" ON locales FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "locales_update_public" ON locales;
CREATE POLICY "locales_update_public" ON locales FOR UPDATE USING (true);
...
CREATE POLICY "calificaciones_insert" ON calificaciones FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "calificaciones_select" ON calificaciones;
CREATE POLICY "calificaciones_select" ON calificaciones FOR SELECT USING (true);

-- ==================== 5. DATOS DE EJEMPLO ====================

-- Insertar negocios demo (solo si no hay datos)
...
SELECT * FROM (VALUES
  ('Pizzeria Napoli', 'Restaurantes', 'Las mejores pizzas artesanales de Magdalena, ingredientes frescos y masa madre.', 'Calle Real #123, Magdalena', '3001112233', 4.8, '25-35 min', 3500, true, true, true),
  ('Super Tienda Don Pepe', 'Tiendas', 'Todo lo que necesitas para tu hogar: abarrotes, limpieza, bebidas y mas.', 'Av. Central #456, Magdalena', '3001113344', 4.3, '20-30 min', 3000, true, true, true),
  ('Licores El Buen Gusto', 'Licoreras', 'Amplia seleccion de cervezas, vinos, licores y cocteles preparados.', 'Carrera 7 #789, Magdalena', '3001114455', 4.6, '30-40 min', 4000, true, false, true),
  ('Drogueria Salud Total', 'Droguerias', 'Medicamentos, productos de cuidado personal y primera necesidad.', 'Calle 12 #321, Magdalena', '3001115566', 4.7, '15-25 min', 2500, true, false, true),
  ('Sushi Master', 'Restaurantes', 'Sushi fresco, rolls especiales y comida japonesa tradicional.', 'Av. del Rio #654, Magdalena', '3001116677', 4.9, '35-50 min', 4500, true, true, true)
) AS v(nombre, categoria, descripcion, direccion, telefono, rating, tiempo_estimado, domicilio_cost, abierto, destacado, activo)
WHERE NOT EXISTS (SELECT 1 FROM negocios LIMIT 1);

-- Productos para Pizzeria Napoli
...
CROSS JOIN (VALUES
  ('Pizza Margherita', 'Mozzarella fresca, albahaca, salsa de tomate', 22000, 'Pizzas'),
  ('Pizza Pepperoni', 'Pepperoni, mozzarella, salsa de tomate', 25000, 'Pizzas'),
  ('Pizza Hawaiana', 'Piña, jamon, mozzarella, salsa de tomate', 24000, 'Pizzas'),
  ('Pizza Vegetariana', 'Pimientos, cebolla, champiñones, aceitunas', 23000, 'Pizzas'),
  ('Coca-Cola 1.5L', 'Gaseosa Coca-Cola personal', 5000, 'Bebidas')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);

-- Productos para Super Tienda Don Pepe
INSERT INTO productos (negocio_id, nombre, descripcion, precio, categoria_producto, disponible)
SELECT n.id, p.nombre, p.descripcion, p.precio, p.cat, true
FROM (SELECT id FROM negocios WHERE nombre = 'Super Tienda Don Pepe') n
CROSS JOIN (VALUES
  ('Arroz Diana 1kg', 'Arroz blanco de alta calidad', 3500, 'Abarrotes'),
  ('Aceite Vegetal 1L', 'Aceite vegetal para cocinar', 8500, 'Abarrotes'),
  ('Leche Entera 1L', 'Leche entera pasteurizada', 4200, 'Lacteos'),
  ('Pan Bimbo Grande', 'Pan de molde integral', 6800, 'Panaderia'),
  ('Jabon para Ropa', 'Detergente en polvo 500g', 4500, 'Limpieza')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);

-- Productos para Licores El Buen Gusto
INSERT INTO productos (negocio_id, nombre, descripcion, precio, categoria_producto, disponible)
SELECT n.id, p.nombre, p.descripcion, p.precio, p.cat, true
FROM (SELECT id FROM negocios WHERE nombre = 'Licores El Buen Gusto') n
CROSS JOIN (VALUES
  ('Cerveza Aguila x6', 'Pack de 6 cervezas Aguila', 18000, 'Cervezas'),
  ('Vino Tinto Casillero', 'Vino tinto Casillero del Diablo 750ml', 45000, 'Vinos'),
  ('Ron Medellin Anejo', 'Ron Medellin Anejo 750ml', 55000, 'Licores'),
  ('Coctel Margarita', 'Coctel Margarita preparado 500ml', 22000, 'Cocteles')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);

-- Productos para Drogueria Salud Total
INSERT INTO productos (negocio_id, nombre, descripcion, precio, categoria_producto, disponible)
SELECT n.id, p.nombre, p.descripcion, p.precio, p.cat, true
FROM (SELECT id FROM negocios WHERE nombre = 'Drogueria Salud Total') n
CROSS JOIN (VALUES
  ('Acetaminofen 500mg x10', 'Analgesico y antipiretico', 2500, 'Medicamentos'),
  ('Ibuprofeno 400mg x10', 'Antiinflamatorio no esteroideo', 3500, 'Medicamentos'),
  ('Curitas x20', 'Apósito adhesivo para heridas', 2000, 'Cuidado Personal'),
  ('Alcohol Antiseptico 500ml', 'Alcohol etilico al 70%', 4500, 'Cuidado Personal')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);
...
CROSS JOIN (VALUES
  ('Roll Salmón Philadelphia', 'Sushi roll con salmon, queso crema y aguacate', 28000, 'Rolls'),
  ('California Roll', 'Cangrejo, aguacate, pepino y masago', 24000, 'Rolls'),
  ('Nigiri Mix x8', '8 piezas variadas de nigiri', 32000, 'Nigiri')
) AS p(nombre, descripcion, precio, cat)
WHERE NOT EXISTS (SELECT 1 FROM productos WHERE negocio_id = n.id LIMIT 1);

-- ==================== 6. VINCULAR LOCALES EXISTENTES ====================

-- Vincular locales existentes con negocios (por nombre)
UPDATE negocios SET local_id = l.id
FROM locales l
WHERE negocios.nombre = l.nombre AND negocios.local_id IS NULL;

-- Crear negocios para locales sin vinculo
...
AND NOT EXISTS (SELECT 1 FROM negocios n WHERE n.nombre = l.nombre);
