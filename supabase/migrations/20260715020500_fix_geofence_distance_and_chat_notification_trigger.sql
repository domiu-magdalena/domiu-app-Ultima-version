-- Corrige bloqueos críticos de GPS y chat detectados durante la estabilización del lanzamiento.
-- 1) Sustituye el operador point <@> point por PostGIS ST_DistanceSphere.
-- 2) Ejecuta el trigger de notificación de mensajes con privilegios controlados.
...
begin
  ...
  if v_business_lat is not null and v_business_lng is not null then
    v_dist_to_business_m := extensions.st_distancesphere(
      extensions.st_setsrid(
        extensions.st_makepoint(new.longitude::double precision, new.latitude::double precision),
        4326
      ),
      extensions.st_setsrid(extensions.st_makepoint(v_business_lng, v_business_lat), 4326)
    );
  end if;

  if v_customer_lat is not null and v_customer_lng is not null then
    v_dist_to_customer_m := extensions.st_distancesphere(
      extensions.st_setsrid(
        extensions.st_makepoint(new.longitude::double precision, new.latitude::double precision),
        4326
      ),
      extensions.st_setsrid(extensions.st_makepoint(v_customer_lng, v_customer_lat), 4326)
    );
  end if;
