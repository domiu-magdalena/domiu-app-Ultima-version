        ...
        <nav className="flex items-center gap-1.5 text-sm text-muted-foreground">
          ...
          {breadcrumbs.map((crumb) => (
            <React.Fragment key={crumb.href}>
              ...
              {crumb.isLast ? (
                <span className="font-medium text-foreground text-sm sm:text-base">{crumb.label}</span>
              ) : (
                <button
                  onClick={() => router.push(crumb.href)}
                  className="hidden sm:block transition-colors hover:text-foreground text-xs"
                >
                  {crumb.label}
                </button>
              )}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-warning/10 to-orange-500/5 text-sm font-medium text-warning transition-all duration-200 hover:from-warning/20 hover:to-orange-500/10 ring-1 ring-warning/10"
          >
            {initials}
          </button>
