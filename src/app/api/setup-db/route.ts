...
-- Súper-App Cliente: stock, propina, cobertura
...
export async function GET() {
  ...
  if (!ref) return NextResponse.json({ error: "No project ref" }, { status: 500 });
  if (!pw) return NextResponse.json({ error: "No DB password configured" }, { status: 500 });

  const errors: string[] = [];
  const logs: string[] = [];

  // Try direct IPv6 connection via db.{ref}.supabase.co
  async function tryDirect() {
    try {
      const dns = await import("dns");
      const addrs = await dns.promises.resolve6(`db.${ref}.supabase.co`);
      const ipv6 = addrs[0];
      logs.push(`Direct IPv6: ${ipv6}`);
      ...
      logs.push("Connected directly via IPv6!");
      await client.query(SQL);
      logs.push("SQL executed successfully!");
      await client.end();
      return true;
    } catch (e: any) {
      logs.push(`Direct failed: ${e.message.slice(0, 150)}`);
      return false;
    }
  }
  ...
  async function tryPooler() {
    const configs = [
      ...
      { host: "aws-0-us-east-1.pooler.supabase.com", port: 6543, user: ref },
      { host: "aws-0-us-east-1.pooler.supabase.com", port: 5432, user: ref },
    ];
    for (const cfg of configs) {
      try {
        logs.push(`Pooler: ${cfg.user}@${cfg.host}:${cfg.port}`);
        const client = new Client({
          host: cfg.host,
          port: cfg.port,
          database: "postgres",
        ...
        await client.connect();
        logs.push("Connected via pooler!");
        await client.query(SQL);
        logs.push("SQL executed successfully!");
        await client.end();
        return true;
      } catch (e: any) {
        logs.push(`Pooler fail: ${e.message.slice(0, 150)}`);
      }
    }
    return false;
  }

  // Try IPv4 from resolved addresses
  async function tryIPv4() {
    try {
      const dns = await import("dns");
      const addrs = await dns.promises.resolve4(`aws-0-us-east-1.pooler.supabase.com`);
      const ip = addrs[0];
      logs.push(`Trying pooler IPv4: ${ip}`);
      const client = new Client({
        host: ip,
        port: 6543,
        database: "postgres",
      ...
      await client.connect();
      logs.push("Connected via pooler IPv4!");
      await client.query(SQL);
      logs.push("SQL executed successfully!");
      await client.end();
      return true;
    } catch (e: any) {
      logs.push(`Pooler IPv4 fail: ${e.message.slice(0, 150)}`);
      return false;
    }
  }

  if (await tryDirect()) {
    return NextResponse.json({ success: true, method: "direct", logs });
  }
  if (await tryPooler()) {
    return NextResponse.json({ success: true, method: "pooler", logs });
  }
  if (await tryIPv4()) {
    return NextResponse.json({ success: true, method: "pooler-ipv4", logs });
  }

  return NextResponse.json({ success: false, error: "Could not connect", logs });
}
