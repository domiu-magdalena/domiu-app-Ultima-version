...
import { Search, MapPin, ChevronRight, Star, Clock, Bike, Bell, Store, Sparkles, ArrowRight, X, CheckCircle, User, Phone, Mail, Lock, TrendingUp, ChevronDown } from "lucide-react";
...
const categories = [
  { label: "Restaurantes", icon: "🍔", slug: "Restaurantes", desc: "Comida rápida, japonesa, pizza y más", gradient: "from-emerald-400/30 to-emerald-600/10" },
  { label: "Mercado", icon: "🛒", slug: "Mercado", desc: "Supermercados, frutas, carnes y despensa", gradient: "from-emerald-500/30 to-emerald-700/10" },
  { label: "Farmacia", icon: "💊", slug: "Droguerias", desc: "Medicamentos 24/7, fórmulas y cuidado", gradient: "from-sky-400/30 to-sky-600/10" },
  { label: "Licores", icon: "🍷", slug: "Licoreras", desc: "Cervezas, vinos, cocteles y más", gradient: "from-purple-400/30 to-purple-600/10" },
  { label: "DomiU Favor", icon: "📦", slug: "Envios", desc: "Mensajería express, lo que necesites", gradient: "from-amber-400/30 to-amber-600/10" },
];

const banners = [
  { title: "50% Dcto en Ceviches", subtitle: "Válido en restaurantes participantes", color: "from-emerald-900/60 to-slate-900/80", emoji: "🦐" },
  { title: "Envío gratis en tu primer pedido", subtitle: "Usa BIENVENIDO · 10% OFF adicional", color: "from-blue-900/60 to-slate-900/80", emoji: "🎉" },
  { title: "Farmacia 24/7 · Abierto ahora", subtitle: "Adjunta tu fórmula médica al pedido", color: "from-teal-900/60 to-slate-900/80", emoji: "💊" },
  { title: "DomiU Express · Mensajería Ya", subtitle: "Lo que necesites, donde sea", color: "from-violet-900/60 to-slate-900/80", emoji: "📦" },
];

const topBuscados = ["Pizza", "Hamburguesa", "Pollo", "Mexicana", "Postres", "Café", "Helado", "Gaseosas"];
        ...
        <>
          ...
          <div className="mt-6 px-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold text-sm text-[#F8FAFC] flex items-center gap-1.5">
                <TrendingUp size={14} className="text-[#10B981]" /> Lo más buscado
              </h2>
              <button onClick={() => router.push("/cliente/negocios")} className="text-[10px] font-bold text-[#10B981] flex items-center gap-1">
            ...
            <div className="flex flex-wrap gap-1.5">
              {topBuscados.slice(0, 8).map(t => (
                <button key={t} onClick={() => router.push(`/cliente/negocios?busqueda=${t}`)}
                  className="px-3 py-1.5 rounded-full text-[11px] font-semibold bg-white/5 text-slate-400 border border-white/10 hover:border-[#10B981]/40 hover:text-[#10B981] active:scale-95 transition-all">
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between px-5 mb-3">
              <h2 className="font-bold text-sm text-[#F8FAFC] flex items-center gap-1.5">
                <Store size={14} className="text-[#10B981]" /> Destacados en Magdalena
              </h2>
              <button onClick={() => router.push("/cliente/negocios")} className="text-[10px] font-bold text-[#10B981] flex items-center gap-1">
            ...
            <div className="p-5">
              {regSuccess ? (
                <div className="text-center py-8">
                  <div className="w-20 h-20 rounded-3xl bg-[#10B981]/10 flex items-center justify-center mx-auto mb-4">
                    <CheckCircle size={40} className="text-[#10B981]" />
                  </div>
                  <h3 className="text-xl font-black text-[#F8FAFC] mb-2">¡Registro exitoso!</h3>
              ...
              {notificaciones.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notificaciones.map(n => (
                    <div key={n.id} className={`p-4 rounded-2xl border ${n.leida ? "bg-white/5 border-white/10" : "bg-[#10B981]/5 border-[#10B981]/20"}`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {!n.leida && <span className="w-2 h-2 rounded-full bg-[#10B981] shrink-0 mt-1.5" />}
                      </div>
                    </div>
                  ))}
                </div>
